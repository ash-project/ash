...
| [`default`](#attributes-attribute-default){: #attributes-attribute-default } | `(-> any) \| mfa \| any` |  | Note: The `default` value may not always be the actual value saved to the resource record for the default case. See the `:constraints` option, the `:allow_nil?` option, and the Ash.Type.* documentation for attribute's type to learn how empty values are handled. |
