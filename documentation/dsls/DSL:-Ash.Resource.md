...
| [`default`](#attributes-attribute-default){: #attributes-attribute-default } | `(-> any) \| mfa \| any` |  | A value to be set on all creates, unless a value is being provided already. Note: if default is `""` or similar blank value, you may need to set `allow_nil? false` and `constraints [allow_empty?: true]` |
