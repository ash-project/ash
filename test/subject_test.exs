defmodule Ash.SubjectTest do
  ...
  describe "set_actor/2 - All Subject Types" do
    test "sets actor directly" do
      actor = %{id: 1, name: "Test User"}

      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_actor(subject, actor)
        assert is_struct(result, subject_type)

        assert result.context.private.actor == actor
      end
    end

    test "sets actor from keyword list" do
      actor = %{id: 1, name: "Test User"}

      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_actor(subject, actor: actor)
        assert is_struct(result, subject_type)
    ...
    test "ignores keyword list without actor key" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_actor(subject, other: "value")
        assert is_struct(result, subject_type)

        refute Map.has_key?(result.context[:private] || %{}, :actor)
      end
    end
    ...
    test "handles empty keyword list" do
      for subject_type <- @subject_types do
        ...
        result = Ash.Subject.set_actor(subject, [])
        assert is_struct(result, subject_type)
  ...
  describe "set_tenant/2 - All Subject Types" do
    test "sets tenant directly" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_tenant(subject, "tenant_123")

        assert result.tenant == "tenant_123"
        assert result.to_tenant == "tenant_123"
      end
    end

    test "sets tenant from keyword list" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_tenant(subject, tenant: "tenant_456")

        assert result.tenant == "tenant_456"
        assert result.to_tenant == "tenant_456"
      end
    end

    test "handles different tenant types" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        # String tenant
        result1 = Ash.Subject.set_tenant(subject, "string_tenant")
        assert result1.tenant == "string_tenant"

        # Atom tenant
        result2 = Ash.Subject.set_tenant(subject, :atom_tenant)
        assert result2.tenant == :atom_tenant
      end
    end
  end

  describe "set_authorize?/2 - All Subject Types" do
    test "sets authorize flag to true" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_authorize?(subject, true)

        assert result.context.private.authorize? == true
      end
    end

    test "sets authorize flag to false" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_authorize?(subject, false)

        assert result.context.private.authorize? == false
      end
    end

    test "sets from keyword list" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_authorize?(subject, authorize?: true)
  ...
  describe "set_tracer/2 - All Subject Types" do
    test "sets tracer module" do
      tracer = MyApp.Tracer

      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_tracer(subject, tracer)

        assert result.context.private.tracer == tracer
      end
    end

    test "sets tracer from keyword list" do
      tracer = [MyApp.Tracer1, MyApp.Tracer2]

      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_tracer(subject, tracer: tracer)

        # Just verify no errors occur and return values are the expected types
        assert is_struct(result, subject_type)
      end
    end

    test "ignores keyword list without tracer key" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_tracer(subject, other: "value")
  ...
  describe "set_result/2 - All Subject Types" do
    test "sets result" do
      result_data = %{id: 1, title: "Test"}

      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.set_result(subject, result_data)

        assert result.context.private.action_result == result_data
      end
    end
  end

  describe "clear_result/1 - All Subject Types" do
    test "clears result" do
      result_data = %{id: 1, title: "Test"}

      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        with_result = Ash.Subject.set_result(subject, result_data)
        assert with_result.context.private.action_result == result_data

        cleared = Ash.Subject.clear_result(with_result)
        refute Map.has_key?(cleared.context[:private] || %{}, :action_result)
      end
    end

    test "clears result when no result exists" do
      for subject_type <- @subject_types do
        subject = new_subject(subject_type)

        result = Ash.Subject.clear_result(subject)

        refute Map.has_key?(result.context[:private] || %{}, :action_result)
      end
    end
  end
