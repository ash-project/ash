...
defmodule Ash.Test.Type.AutoTypePubSubTest do
  ...
  defmodule PubSubTag do
    ...
    relationships do
      belongs_to :pub_sub_post, Ash.Test.Type.AutoTypePubSubTest.PubSubPost do
        public?(true)
      end
    end
  end
  ...
  defmodule PubSubPost do
    ...
    calculations do
      ...
      calculate :metadata_list, {:array, :string}, expr(fragment("?", title)), public?: true

      calculate :card, :auto, expr(%{title: title, view_count: view_count, active: active}),
        public?: true
    end
  end
  ...
  defmodule ComplexPost do
    ...
    calculations do
      calculate :current_priority, :auto, expr(priority), public?: true
      calculate :current_metadata, :auto, expr(metadata), public?: true
      # get_path can't auto-resolve; keep explicit type
      calculate :current_address, Address, expr(metadata[:address]), public?: true
      calculate :current_content, :auto, expr(content), public?: true
      calculate :wrapped_title, :auto, expr(short_title), public?: true
      calculate :metadata_copies, :auto, expr(all_metadata), public?: true
      calculate :priority_list, :auto, expr(all_priorities), public?: true
      calculate :current_score, :auto, expr(score), public?: true
      calculate :current_id, :auto, expr(id), public?: true
      calculate :current_published_at, :auto, expr(published_at), public?: true
      calculate :current_slug, :auto, expr(slug), public?: true
    end
  end
  ...
  describe "broadcast values" do
    ...
    test "aggregate calculation broadcasts correctly" do
      PubSubPost
      |> Ash.Changeset.for_create(:create, %{title: "Tagged", body: "Post"})
      |> Ash.create!()
