...
defmodule Reactor.Step.Recurse.Mermaid do
  @moduledoc """
  ...
  def to_mermaid(%{impl: {module, opts}} = step, options) do
    ...
    with {:ok, sub_graph} <- Reactor.to_mermaid(reactor, options),
         {:ok, node} <- describe_step(step, module, max_iterations, options) do
  ...
  defp describe_step(step, module, max_iterations, options) do
