...
defmodule Ash.SatSolver.Expression do
  @moduledoc """
  ...
  ## Examples
      ...
      iex> b((:a and not :b) or (not :c and :d))
  ...
  ## Examples
      ...
      iex> b(:a and not :b)
      iex> |> to_algebra()
      iex> |> Inspect.Algebra.format(80)
      iex> |> IO.iodata_to_binary()
      ":a and not :b"

      # You can customize how variables are represented:
      iex> b(:a and not :b)
      iex> |> to_algebra([], fn var -> quote do: unquote(var)() end)
      iex> |> Inspect.Algebra.format(80)
      iex> |> IO.iodata_to_binary()
      "a() and not b()"
  ...
  ## Examples
      ...
      iex> {_result, acc} = prewalk(b(:a and :b), [], fn
      ...
      iex> {_result, count} = prewalk(b(:a and (:b or :c)), 0, fn
  ...
  ## Examples
      ...
      iex> {_result, acc} = postwalk(b(:a and :b), [], fn
      ...>   {:and, _, _} = expr, acc -> {expr, [:and | acc]}
      ...>   other, acc -> {other, acc}
      ...> end)
      iex> acc
      ...
      iex> {_result, count} = postwalk(b(:a and (:b or :c)), 0, fn
      ...>   {:and, _, _} = expr, acc -> {expr, acc + 1}
      ...>   {:or, _, _} = expr, acc -> {expr, acc + 1}
      ...>   other, acc -> {other, acc}
      ...> end)
  ...
  ## Examples
      ...
      iex> is_in_cnf?(b(:a and :b))
      true

      iex> is_in_cnf?(b(:a or :b))
      true

      iex> is_in_cnf?(b((:a or :b) and :c))
      true

      iex> is_in_cnf?(b(:a or (:b and :c)))
      false

      iex> is_in_cnf?(b(not (:a and :b)))
      false

      iex> is_in_cnf?(b(not :a))
      true

  """
  @spec is_in_cnf?(expression :: t(variable)) :: boolean() when variable: term()
  def is_in_cnf?(expr) do
  ...
  defp do_expand(other, acc, callback),
  ...
  ## Examples
      ...
      iex> all_or_none([:begin_transaction, :commit_transaction])
      {:not,
      ...
      iex> all_or_none([:dark_mode_ui, :dark_mode_api])
      {:not,
       {:and, {:not, {:and, :dark_mode_api, :dark_mode_ui}},
        {:or, :dark_mode_api, :dark_mode_ui}}}
  ...
  ## Examples
      ...
      iex> exactly_one([:light_theme, :dark_theme])
      {:and, {:not, {:and, :dark_theme, :light_theme}},
       {:or, :dark_theme, :light_theme}}
