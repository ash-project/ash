defmodule Ash.Actions.Update.Bulk do
  ...
  def run(api, stream, action, input, opts) do
    ...
    if !resource do
      raise ArgumentError,
            "Could not determine resource for bulk update. Please provide the `resource` option if providing a stream of inputs."
    end
  ...
  defp pre_template_all_changes(action, resource, :update, base, actor) do
    action.changes
  ...
  defp setup_changeset(
    ...
    |> Map.put(:api, api)
    |> Ash.Changeset.prepare_changeset_for_action(action, opts)
    |> Ash.Changeset.put_context(:bulk_update, %{index: index})
      ...
      if changes[index] == :all do
        ...
        {matches, non_matches} =
          ...
          |> Enum.split_with(fn
            ...
            changeset ->
              changeset.context.bulk_update.index in List.wrap(changes[index])
          end)
  ...
  defp sort(%{records: records} = result, opts) when is_list(records) do
    if opts[:sorted?] do
      %{result | records: Enum.sort_by(records, & &1.__metadata__.bulk_update_index)}
    else
      result
    end
  end
  ...
  defp run_after_action_hooks(
    ...
    Enum.flat_map(batch_results, fn result ->
      changeset = changesets_by_index[result.__metadata__.bulk_update_index]
      ...
      if changes[index] == :all do
        results =
          Enum.map(results, fn result ->
            {changesets_by_index[result.__metadata__.bulk_update_index], result}
          end)
        ...
        {matches, non_matches} =
          ...
          |> Enum.split_with(fn
            {:ok, result} ->
              result.__metadata__.bulk_update_index in List.wrap(changes[index])
        ...
        matches =
          Enum.map(matches, fn match ->
            {changesets_by_index[match.__metadata__.bulk_update_index], match}
          end)
                ...
                | must_return_records?: must_return_records?,
                  ...
                  changes:
                    Map.put(
                      state.changes,
                      change_index,
                      Enum.map(matches, & &1.context.bulk_update.index)
                    )
              }
            end
          end
