defmodule Ash.Domain do
  ...
  @stream_opts [
                 ...
                 allow_stream_with: [
                   ...
                   doc:
                     "The 'worst' strategy allowed to be used to fetch records. See `Ash.Domain.stream!/2` docs for more.",
                   default: :keyset
                 ],
                 stream_with: [
                   type: {:one_of, [:keyset, :offset, :full_read]},
                   doc:
                     "The specific strategy to use to fetch records. See `Ash.Domain.stream!/2` docs for more."
                 ]
               ]
               |> Spark.Options.merge(
                 @read_opts_schema,
  ...
  @bulk_update_opts_schema [
                             ...
                             allow_stream_with: [
                               type: {:one_of, [:keyset, :offset, :full_read]},
                               doc:
                                 "The 'worst' strategy allowed to be used to fetch records if the `:stream` strategy is chosen. See `Ash.Domain.stream!/2` docs for more.",
                               default: :keyset
                             ],
                             authorize_query?: [
                               type: :boolean,
  ...
  def aggregate(domain, query, aggregate_or_aggregates, opts \\ []) do
    query = Ash.Query.new(query)
    opts = Spark.Options.validate!(opts, @aggregate_opts)

    Ash.Actions.Aggregate.run(domain, query, List.wrap(aggregate_or_aggregates), opts)
  end
  ...
  @spec can?(
          ...
          boolean | no_return
  def can?(domain, action_or_query_or_changeset, actor, opts \\ []) do
    opts =
      opts
      |> Keyword.put_new(:maybe_is, true)
      |> Keyword.put_new(:filter_with, :filter)

    case can(domain, action_or_query_or_changeset, actor, opts) do
  ...
  defp or_query(query, resource, domain) do
    query || Ash.Query.new(resource, domain)
  end
  ...
  @spec run_action!(domain :: Ash.Domain.t(), input :: Ash.ActionInput.t(), opts :: Keyword.t()) ::
          term | no_return
  def run_action!(domain, input, opts \\ []) do
    domain
  ...
  def run_action(domain, input, opts \\ []) do
    case Spark.Options.validate(opts, @run_action_opts) do
      {:ok, opts} ->
        ...
        Ash.Actions.Action.run(domain, input, opts)
  ...
  def calculate(resource_or_record, calculation, opts \\ []) do
    ...
    with {:ok, opts} <- Spark.Options.validate(opts, Ash.Domain.calculate_opts()),
         {:calc,
          %{
  ...
  @doc false
  @spec get!(Ash.Domain.t(), Ash.Resource.t(), term(), Keyword.t()) ::
          Ash.Resource.record() | no_return
  def get!(domain, resource, id, opts \\ []) do
  ...
  @doc false
  def page!(domain, keyset, request) do
    domain
    |> page(keyset, request)
    |> unwrap_or_raise!()
  end
  ...
  def page(domain, %Ash.Page.Keyset{results: [], before: before, rerun: {query, opts}}, :prev)
    ...
    read(domain, query, Keyword.put(opts, :page, new_page_opts))
  end
  ...
  def page(
    ...
    case read(domain, query, Keyword.put(opts, :page, new_page_opts)) do
      {:ok, %{results: []}} ->
        {:ok, page}
  ...
  def page(domain, %Ash.Page.Keyset{results: results, rerun: {query, opts}} = page, :prev) do
    first_keyset =
      results
      |> List.first()
      |> Map.get(:__metadata__)
  ...
  def page(domain, %Ash.Page.Keyset{rerun: {query, opts}}, :first) do
    page_opts =
    ...
    read(domain, query, Keyword.put(opts, :page, page_opts))
  end

  def page(domain, %Ash.Page.Keyset{rerun: {query, opts}}, :self) do
    read(domain, query, opts)
  end
  ...
  @doc false
  def load!(domain, data, query, opts \\ []) do
  ...
  @doc false
  def load(domain, data, query, opts \\ [])
  def load(_, [], _, _), do: {:ok, []}
  def load(_, nil, _, _), do: {:ok, nil}
  def load(_, {:error, error}, _, _), do: {:error, error}

  def load(domain, {:ok, values}, query, opts) do
    resource = resource_from_data!(values, query, opts)
    load(domain, values, query, Keyword.put(opts, :resource, resource))
  end

  def load(domain, %struct{results: results} = page, query, opts)
  ...
  def load(domain, data, query, opts) when not is_list(data) do
  ...
  def load(domain, [record | _] = data, query, opts) do
    resource = resource_from_data!(data, query, opts)
    ...
    query = Map.put(query, :domain, domain)
  ...
  @spec stream!(domain :: module(), query :: Ash.Query.t(), opts :: Keyword.t()) ::
          Enumerable.t(Ash.Resource.record())
  def stream!(domain, query, opts \\ []) do
  ...
  @doc false
  @spec read!(Ash.Domain.t(), Ash.Query.t() | Ash.Resource.t(), Keyword.t()) ::
          list(Ash.Resource.record()) | Ash.Page.page() | no_return
  def read!(domain, query, opts \\ []) do
  ...
  def read(domain, query, opts \\ [])

  def read(domain, resource, opts) when is_atom(resource) do
    read(domain, Ash.Query.new(resource, domain), opts)
  end

  def read(domain, query, opts) do
    query = Ash.Query.set_domain(query, domain)
  ...
  @doc false
  def read_one!(domain, query, opts) do
    domain
    |> read_one(query, opts)
    |> unwrap_or_raise!()
  end
  ...
  @doc false
  @spec create!(Ash.Domain.t(), Ash.Changeset.t(), Keyword.t()) ::
          Ash.Resource.record()
          | {Ash.Resource.record(), list(Ash.Notifier.Notification.t())}
          | no_return
  def create!(domain, changeset, opts) do
    domain
  ...
  def create(domain, changeset, opts) do
    with {:ok, opts} <- Spark.Options.validate(opts, @create_opts_schema),
      ...
      Create.run(domain, changeset, action, opts)
    end
  end

  @doc false
  @spec bulk_create!(Ash.Domain.t(), Enumerable.t(map), Ash.Resource.t(), atom, Keyword.t()) ::
          Ash.BulkResult.t() | no_return
  def bulk_create!(domain, inputs, resource, action, opts) do
    domain
    ...
    case inputs do
      ...
      inputs ->
        case Spark.Options.validate(opts, @bulk_create_opts_schema) do
          {:ok, opts} ->
            Create.Bulk.run(domain, resource, action, inputs, opts)
  ...
  @spec bulk_update!(
          ...
          Ash.BulkResult.t() | no_return
  def bulk_update!(domain, stream_or_query, action, input, opts) do
    domain
  ...
  @spec bulk_update(
          ...
          Ash.BulkResult.t()
  def bulk_update(domain, query_or_stream, action, input, opts) do
    case query_or_stream do
      ...
      query_or_stream ->
        case Spark.Options.validate(opts, @bulk_update_opts_schema) do
          {:ok, opts} ->
            Update.Bulk.run(domain, query_or_stream, action, input, opts)
  ...
  @spec bulk_destroy!(
          ...
          Ash.BulkResult.t() | no_return
  def bulk_destroy!(domain, stream_or_query, action, input, opts) do
    domain
  ...
  @spec bulk_destroy(
          ...
          Ash.BulkResult.t()
  def bulk_destroy(domain, query_or_stream, action, input, opts) do
    case query_or_stream do
      [] ->
        result = %Ash.BulkResult{status: :success, errors: []}
      ...
      query_or_stream ->
        case Spark.Options.validate(opts, @bulk_destroy_opts_schema) do
          {:ok, opts} ->
            %Ash.BulkResult{} =
              result = Destroy.Bulk.run(domain, query_or_stream, action, input, opts)
  ...
  @doc false
  def update!(domain, changeset, opts) do
  ...
  @doc false
  @spec update(Ash.Domain.t(), Ash.Changeset.t(), Keyword.t()) ::
          {:ok, Ash.Resource.record()}
          | {:ok, Ash.Resource.record(), list(Ash.Notifier.Notification.t())}
          | {:error, term}
  def update(domain, changeset, opts) do
    with {:ok, opts} <- Spark.Options.validate(opts, @update_opts_schema),
      ...
      Update.run(domain, changeset, action, opts)
    end
  end

  @doc false
  @spec destroy!(Ash.Domain.t(), Ash.Changeset.t() | Ash.Resource.record(), Keyword.t()) ::
          :ok
          | Ash.Resource.record()
          | list(Ash.Notifier.Notification.t())
          | {Ash.Resource.record(), list(Ash.Notifier.Notification.t())}
          | no_return()
  def destroy!(domain, changeset, opts) do
  ...
  @spec destroy(Ash.Domain.t(), Ash.Changeset.t() | Ash.Resource.record(), Keyword.t()) ::
          ...
          | {:ok, Ash.Resource.record(), list(Ash.Notifier.Notification.t())}
          | {:error, term}
  def destroy(domain, %Ash.Changeset{resource: resource} = changeset, opts) do
    with {:ok, opts} <- Spark.Options.validate(opts, @destroy_opts_schema),
      ...
      Destroy.run(domain, changeset, action, opts)
    end
  end

  def destroy(domain, record, opts) do
    destroy(domain, Ash.Changeset.new(record), opts)
  end
